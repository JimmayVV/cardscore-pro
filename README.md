...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create cardscore-pro
  fly apps create cardscore-pro-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app cardscore-pro
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app cardscore-pro-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'cardscore-pro-staging' does not match app name in config file 'cardscore-pro'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name cardscore-pro-db
  fly postgres attach --app cardscore-pro cardscore-pro-db

  fly postgres create --name cardscore-pro-staging-db
  fly postgres attach --app cardscore-pro-staging cardscore-pro-staging-db
  ```
